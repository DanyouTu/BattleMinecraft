...
在 Minecraft 中還原《戰地風雲》的經典佔點模式！本資料包支援多人連線，具備據點佔領、專屬重生點、小隊系統與戰場攝影機預覽系統。
...
- 🎖️ **職業系統：** 提供 5 種職業 — 突擊、醫療、支援、偵察、刺客。
- 👫 **小隊系統：** 最多 4 人一組，可預覽隊友視角並直接部署至隊友身邊。
- ✅ **開局驗證：** 自動檢查基地、隊伍、據點是否正確設定，避免無效開局。
...
> 第一項設定必做，否則系統不起作用，且設定時務必靠近點位的盔甲架 (距離 3 格以內)
 ...
 3. **命名據點：** 輸入以下指令來為據點命名與綁定。以下指令以設定「A點」為例，你可以自由修改裡面的`A點`與顏色 `gold`：
   ...
   ```
### Step 5: 分配隊伍與開始遊戲
玩家可透過遊戲內聊天選單自行加入隊伍（不需 OP 權限），或由管理員手動分配。一切就緒後，輸入以下指令正式開始遊戲：
```mcfunction
/function bf:game/game_start

```
> [!NOTE]
> 系統會在開局時自動驗證設定（基地、隊伍等），若有缺漏會顯示錯誤訊息。

## 👫 小隊系統

遊戲開始後，在大廳中的玩家可透過部署選單底部的翻頁按鈕切換至**小隊頁面**：
- **加入小隊：** 點擊加入 4 個小隊之一（每隊最多 4 人）。
- **預覽隊友：** 以旁觀者視角觀看存活隊友的位置。
- **部署至隊友：** 直接傳送至存活隊友身邊重生。
